...
async def get_intervention_type_config(type_name: str, user=Depends(get_current_user), db=Depends(get_tenant_db)):
    """Get appointment configuration for a specific intervention type."""
    settings = await db.school_settings.find_one({}, {"_id": 0}) or {}
    types = settings.get("intervention_types") or SETTINGS_DEFAULTS.get("intervention_types", [])

    # Find the intervention type config
    type_config = None
    for t in types:
        if isinstance(t, str) and t == type_name:
            type_config = {"name": t}
            break
        elif isinstance(t, dict) and t.get("name") == type_name:
            type_config = t
            break
    ...
    # Return appointment config or defaults
    default_config = {
    ...
    if isinstance(type_config, dict) and type_config.get("appointment_config"):
        config = type_config["appointment_config"]
        # Merge with defaults for any missing fields
        return {
            "rooms": config.get("rooms", default_config["rooms"]),
            "statuses": config.get("statuses", default_config["statuses"]),
            "flags": config.get("flags", default_config["flags"]),
        }
...
async def export_all_data(user=Depends(get_current_user), db=Depends(get_tenant_db)):
    ...
    collections = [
        ...
        "interventions", "case_notes",
        # Appointments system
        "appointments",
        # Alerts and notifications
        "alerts", "notifications",
        # School settings and administration
        "school_settings", "users", "user_sessions",
        # Team collaboration
        "tasks",
