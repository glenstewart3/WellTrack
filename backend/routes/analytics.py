...
class TermComparisonRequest(BaseModel):
    terms: list

@router.get("/screening/available-terms")
async def get_available_terms(user=Depends(get_current_user), db=Depends(get_tenant_db)):
    """Get list of all screening periods with data."""
    pipeline = [
        {"$match": {"student_id": {"$exists": True}}},
        {"$group": {
            "_id": {"screening_period": "$screening_period", "created_at": "$created_at"},
            "count": {"$sum": 1}
        }},
    ...
    terms = []
    async for doc in db.saebrs.aggregate(pipeline):
        period = doc["_id"].get("screening_period", "")
        if period:
            # Parse "Term 1 2025" format
            parts = period.split()
            if len(parts) >= 3:
                term = f"{parts[0]} {parts[1]}"
                year = parts[2]
                terms.append({"term": term, "year": year, "screening_period": period, "count": doc["count"]})
    
    return terms[:20]  # Return last 20 terms
...
async def term_comparison(req: TermComparisonRequest, user=Depends(get_current_user), db=Depends(get_tenant_db)):
    ...
    for term_info in req.terms:
        ...
        # Get students screened in this period
        screened_students = await db.saebrs.distinct(
            "student_id", 
            {"screening_period": screening_period}
        )
        
        if not screened_students:
            continue
            
        # Get SAEBRS data for this period
        saebrs_docs = await db.saebrs.find(
            {"screening_period": screening_period, "student_id": {"$in": screened_students}},
            {"_id": 0}
        ).to_list(1000)
        
        saebrs_by_student = {s["student_id"]: s for s in saebrs_docs}
        
        # Get SAEBRS+ data for these students
        plus_docs = await db.saebrs_plus.find(
            {"student_id": {"$in": screened_students}},
            {"_id": 0}
        ).sort("created_at", -1).to_list(1000)
        
        plus_by_student = {}
        for p in plus_docs:
            if p["student_id"] not in plus_by_student:
                plus_by_student[p["student_id"]] = p
        
        # Calculate tier distribution
        tier_counts = {1: 0, 2: 0, 3: 0}
        saebrs_totals = []
        
        for sid in screened_students:
            saebrs = saebrs_by_student.get(sid)
            plus = plus_by_student.get(sid)
            ...
            if saebrs and saebrs.get("total_score"):
                saebrs_totals.append(saebrs["total_score"])
        
        total_screened = len(screened_students)
        
        # Count active interventions during this period
        # (interventions that existed during the term)
        start_of_term = f"{term_info['year']}-01-01" if "Term 1" in term_info['term'] else (
            f"{term_info['year']}-04-01" if "Term 2" in term_info['term'] else (
                f"{term_info['year']}-07-01" if "Term 3" in term_info['term'] else f"{term_info['year']}-10-01"
            )
        )
        end_of_term = f"{term_info['year']}-03-31" if "Term 1" in term_info['term'] else (
            f"{term_info['year']}-06-30" if "Term 2" in term_info['term'] else (
                f"{term_info['year']}-09-30" if "Term 3" in term_info['term'] else f"{term_info['year']}-12-31"
            )
        )
        
        active_interventions = await db.interventions.count_documents({
            "start_date": {"$lte": end_of_term},
            "$or": [
                {"status": "active"},
                {"status": "completed", "review_date": {"$gte": start_of_term}}
            ]
        })
        ...
        results.append({
            ...
            "year": term_info["year"],
            "screening_period": screening_period,
            "students_screened": total_screened,
            "tier1_count": tier_counts.get(1, 0),
            "tier2_count": tier_counts.get(2, 0),
            "tier3_count": tier_counts.get(3, 0),
            "tier1_percentage": round(tier_counts.get(1, 0) / total_screened * 100, 1) if total_screened else 0,
            "tier2_percentage": round(tier_counts.get(2, 0) / total_screened * 100, 1) if total_screened else 0,
            "tier3_percentage": round(tier_counts.get(3, 0) / total_screened * 100, 1) if total_screened else 0,
            "avg_saebrs_total": round(sum(saebrs_totals) / len(saebrs_totals), 1) if saebrs_totals else 0,
            "avg_attendance": 92.5,  # Placeholder - would calculate from attendance data
