...
- **Early-years self-report age bands fixed** — `isF2Student` now robustly matches Foundation, Year 1 and Year 2 regardless of zero-padded SIS values (`Year 01`, `Year 02`, `01`, `02`). Year 3+ correctly routes to the standard self-report.
- **Early-years illustrations refreshed** — 7 new SVG illustrations (Q1–Q7) shipped as static assets under `/public/early-years/`. Illustration components reduced to thin `<img>` wrappers so future art refreshes are file-drop-only.
