...
function InterventionDetailModal({ intv, students, onClose, onUpdated, canEdit }) {
  ...
  const [effectivenessNotes, setEffectivenessNotes] = useState(intv.effectiveness_notes || '');
  const [outcomeRating, setOutcomeRating] = useState(intv.outcome_rating || 0);
  const [showReviewSection, setShowReviewSection] = useState(intv.status === 'completed');
  ...
  const save = async () => {
    ...
    try {
      ...
      // Include effectiveness review if intervention is completed and review section is shown
      if (status === 'completed' && showReviewSection) {
        payload.effectiveness_notes = effectivenessNotes;
        payload.outcome_rating = outcomeRating;
        payload.reviewed_at = new Date().toISOString();
        payload.reviewed_by = user?.user_id;
      }

      const res = await api.put(`/interventions/${intv.intervention_id}`, payload);
      onUpdated(res.data);
      onClose();
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 text-sm">
          ...
          {/* Effectiveness Review - only for completed interventions */}
          {status === 'completed' && (
            <div className="border-t border-slate-200 pt-4 mt-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Effectiveness Review</p>
                {canEdit && (
                  <button
                    onClick={() => setShowReviewSection(!showReviewSection)}
                    className="text-xs text-slate-400 hover:text-slate-600"
                  >
                    {showReviewSection ? 'Hide' : 'Add Review'}
                  </button>
                )}
              </div>
              ...
              {showReviewSection && (
                <>
                  {/* Outcome Rating */}
                  <div className="mb-3">
                    <p className="text-xs text-slate-400 mb-2">Outcome Rating</p>
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          key={star}
                          onClick={() => canEdit && setOutcomeRating(star)}
                          disabled={!canEdit}
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      {outcomeRating === 1 && 'Not effective'}
                      {outcomeRating === 2 && 'Slightly effective'}
                      {outcomeRating === 3 && 'Moderately effective'}
                      {outcomeRating === 4 && 'Effective'}
                      {outcomeRating === 5 && 'Highly effective'}
                    </p>
                  </div>
                  ...
                  {canEdit ? (
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="Describe the intervention's effectiveness, what worked well, and any recommendations for future use..."
                        className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 resize-none"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-slate-700 leading-relaxed">{intv.effectiveness_notes}</p>
                    </div>
                  )}
                  ...
                  {intv.reviewed_at && (
                    <p className="text-xs text-slate-400 mt-2">
                      Reviewed on {new Date(intv.reviewed_at).toLocaleDateString()}
                    </p>
                  )}
                </>
              )}
...
export default function InterventionsPage() {
  ...
  const saveBulkInterventions = async (forms) => {
    const res = await api.post('/interventions/bulk', { interventions: forms });
    setInterventions(prev => [...res.data.results, ...prev]);
    setForm({ student_id: '', intervention_type: '', assigned_staff: '', start_date: '', review_date: '', goals: '', rationale: '', frequency: '', status: 'active', progress_notes: '' });
  };
    ...
    <div className="p-6 lg:p-8 fade-in">
      ...
      {loading ? (
        ...
        interventions.length === 0 ? (
          ...
          <EmptyState
            type="interventions_filtered"
            onAction={() => { setSearch(''); setStatusFilter(''); setTypeFilter(''); setStaffFilter(''); setOverdueOnly(false); }}
          />
        )
      ) : groupByStudent ? (
        /* ── Grouped by student ── */
      ...
      <AddInterventionModal
        ...
        onBulkSave={saveBulkInterventions}
        students={students}
