...
const PROGRESS_OPTS = [
  { value: 'on_track',    label: 'On Track',      cls: 'bg-teal-100 text-teal-700 ring-1 ring-teal-300' },
...
function TierBadge({ tier }) {
  if (!tier) return null;
  const cls = ['','bg-teal-100 text-teal-700','bg-amber-100 text-amber-700','bg-red-100 text-red-700'];
...
function StudentAvatar({ student, size = 'sm' }) {
  ...
  if (!student) return (
    <div className={`${dim} rounded-full bg-stone-100 flex items-center justify-center shrink-0`}>
...
function SortTh({ col, label, sortBy, sortDir, onSort }) {
  ...
  return (
    <th onClick={() => onSort(col)}
      className="text-left py-3 px-4 text-xs font-semibold text-stone-400 uppercase tracking-wider whitespace-nowrap cursor-pointer hover:text-stone-700 select-none transition-colors">
      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-start mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-stone-900 text-base">{student ? studentDisplayName(student) : intv.student_id}</p>
              <div className="flex items-center gap-2 mt-0.5">
                <TierBadge tier={student?.mtss_tier} />
                {student && <span className="text-xs text-stone-400">{student.class_name}</span>}
              </div>
            </div>
          </div>
          <button onClick={onClose}><X size={18} className="text-stone-400 hover:text-stone-700" /></button>
        </div>
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <p className="font-medium text-stone-800">{intv.intervention_type}</p>
            </div>
            <div>
              <p className="text-xs text-stone-400 mb-0.5">Assigned Staff</p>
              <p className="font-medium text-stone-800">{intv.assigned_staff}</p>
            </div>
            <div>
              <p className="text-xs text-stone-400 mb-0.5">Start Date</p>
              <p className="font-medium text-stone-800">{intv.start_date || '—'}{days != null && <span className="text-stone-400 text-xs ml-1">({days}d)</span>}</p>
            </div>
            <div>
              <p className="text-xs text-stone-400 mb-0.5">Review Date</p>
              <p className={`font-medium ${overdue ? 'text-red-600' : 'text-stone-800'}`}>{intv.review_date || '—'}</p>
            </div>
            {intv.frequency && <div className="col-span-2">
              <p className="text-xs text-stone-400 mb-0.5">Frequency</p>
              <p className="font-medium text-stone-800">{intv.frequency}</p>
            </div>}
          </div>

          {intv.rationale && <div>
            <p className="text-xs text-stone-400 mb-0.5">Rationale</p>
            <p className="text-stone-700 leading-relaxed">{intv.rationale}</p>
          </div>}

          {intv.goals && <div>
            <p className="text-xs text-stone-400 mb-0.5">Goals</p>
            <p className="text-stone-700 leading-relaxed whitespace-pre-wrap">{intv.goals}</p>
          </div>}

          {/* Goal Progress Status */}
          {canEdit && (
            <div>
              <p className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Goal Progress</p>
              <div className="flex flex-wrap gap-2">
                {PROGRESS_OPTS.map(opt => (
                  <button key={opt.value}
                    onClick={() => setProgressStatus(p => p === opt.value ? '' : opt.value)}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                      progressStatus === opt.value ? `${opt.cls} shadow-sm` : 'bg-stone-100 text-stone-500 hover:bg-stone-200'
                    }`}>
                    {opt.label}
                  </button>
                ))}
          ...
          {!canEdit && progressStatus && (
            <div>
              <p className="text-xs text-stone-400 mb-1">Goal Progress</p>
              <span className={`px-3 py-1 rounded-full text-xs font-semibold ${PROGRESS_OPTS.find(o => o.value === progressStatus)?.cls || 'bg-stone-100 text-stone-600'}`}>
          ...
          {canEdit ? (
            <div>
              <p className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-1">Progress Notes</p>
              <textarea rows={3} value={notes} onChange={e => setNotes(e.target.value)}
                placeholder="Add progress notes..."
                className="w-full px-3 py-2.5 text-sm border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-stone-300 resize-none" />
            </div>
          ) : intv.progress_notes && (
            <div>
              <p className="text-xs text-stone-400 mb-0.5">Progress Notes</p>
              <p className="text-stone-700 leading-relaxed">{intv.progress_notes}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2 mt-5">
          ...
          <button onClick={() => navigate(`/students/${intv.student_id}`)}
            className="flex items-center gap-1.5 px-4 py-3 bg-stone-100 text-stone-700 text-sm font-medium rounded-xl hover:bg-stone-200 transition-colors">
            <ArrowUpRight size={15} /> Profile
          </button>
          <button onClick={onClose}
            className="px-4 py-3 bg-stone-100 text-stone-700 text-sm font-medium rounded-xl hover:bg-stone-200 transition-colors">
            Close
          </button>
        </div>
      </div>
...
export default function InterventionsPage() {
    ...
    return (
      <tr key={intv.intervention_id}
        ...
        className={`border-b border-stone-50 transition-colors cursor-pointer ${
          overdue ? 'bg-red-50/60 hover:bg-red-50' : 'hover:bg-stone-50'
        }`}>
        ...
        <td className="py-3 px-4">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <p className="font-medium text-stone-900 truncate">{getName(intv.student_id)}</p>
              <div className="flex items-center gap-1.5 mt-0.5">
                <TierBadge tier={student?.mtss_tier} />
                {student?.class_name && <span className="text-xs text-stone-400">{student.class_name}</span>}
              </div>
            </div>
          </div>
        </td>
        ...
        {/* Type */}
        <td className="py-3 px-4">
          <span className="bg-stone-100 text-stone-700 text-xs px-2.5 py-1 rounded-full font-medium whitespace-nowrap">{intv.intervention_type}</span>
        </td>

        {/* Staff */}
        <td className="py-3 px-4 text-stone-600 text-xs whitespace-nowrap">{intv.assigned_staff}</td>

        {/* Days active */}
        <td className="py-3 px-4">
          {days != null
            ? <span className="text-xs font-medium text-stone-500 bg-stone-100 px-2 py-0.5 rounded-full whitespace-nowrap">Day {days}</span>
        ...
        <td className="py-3 px-4 text-xs whitespace-nowrap">
          {intv.review_date
            ? <span className={overdue ? 'text-red-600 font-semibold' : dueWeek ? 'text-amber-600 font-medium' : 'text-stone-500'}>
                {intv.review_date}
              </span>
        ...
        <td className="py-3 px-4">
          {prog
            ? <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${prog.cls.replace('ring-1 ring-teal-300','').replace('ring-1 ring-amber-300','').replace('ring-1 ring-red-300','').replace('ring-1 ring-blue-300','')}`}>{prog.label}</span>
            : <span className="text-xs text-stone-300">—</span>}
        </td>
        ...
        <td className="py-3 px-4">
          <div className="flex items-center gap-2">
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${
              intv.status === 'active'    ? 'bg-teal-100 text-teal-700' :
              intv.status === 'completed' ? 'bg-blue-100 text-blue-700'      : 'bg-stone-100 text-stone-600'
            }`}>{intv.status}</span>
            {canDo('interventions.add_edit') && (
              <button onClick={e => quickToggle(e, intv)}
                title={intv.status === 'active' ? 'Mark completed' : 'Reactivate'}
                className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-stone-200 transition-all text-stone-400 hover:text-stone-700">
                <CheckCircle size={13} />
              </button>
            )}
          </div>
        ...
        <td className="py-3 px-4" onClick={e => { e.stopPropagation(); navigate(`/students/${intv.student_id}`); }}>
          <button className="p-1.5 rounded-lg hover:bg-indigo-50 text-stone-300 hover:text-indigo-500 transition-colors">
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto fade-in">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
        <div>
          <h1 className="text-3xl font-bold text-stone-900 flex items-center gap-3" style={{ fontFamily: 'Manrope,sans-serif' }}>
            <Target size={28} className="text-stone-600" /> Interventions
          </h1>
          {/* Stat chips */}
          <div className="flex flex-wrap items-center gap-2 mt-2">
            <span className="text-xs font-semibold px-2.5 py-1 bg-teal-100 text-teal-700 rounded-full">{activeCount} active</span>
            {overdueCount > 0 && (
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setGroupByStudent(g => !g)}
            ...
            className={`flex items-center gap-2 px-3 py-2 border rounded-xl text-sm font-medium transition-colors ${groupByStudent ? 'bg-stone-900 text-white border-stone-900' : 'bg-white border-stone-200 text-stone-700 hover:bg-stone-50'}`}>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
          ...
          {[['', 'All'], ['active', 'Active'], ['overdue', 'Overdue'], ['completed', 'Completed']].map(([val, label]) => (
            <button key={val} onClick={() => setFilterStatus(val)}
              className={`px-4 py-2 text-sm font-medium transition-colors flex items-center gap-1.5 ${
                filterStatus === val
                  ? val === 'overdue' ? 'bg-red-600 text-white' : 'bg-stone-900 text-white'
        ...
        <div className="relative">
          ...
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            ...
            className="pl-8 pr-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none bg-white w-44" />
        </div>

        {/* Type filter */}
        <select value={filterType} onChange={e => setFilterType(e.target.value)}
          className="px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none bg-white">
      ...
      <p className="text-xs text-stone-400 mb-3">Click any row to view and edit details · <kbd className="bg-stone-100 px-1 rounded">→</kbd> icon goes to student profile</p>

      {/* Content */}
      {loading ? (
        <div className="p-8 space-y-3">{[1,2,3,4].map(i => <div key={i} className="h-16 bg-stone-100 rounded animate-pulse" />)}</div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {grouped.map(([sid, intvsForStudent]) => {
            ...
            return (
              <div key={sid} className="bg-white border border-stone-200 rounded-xl overflow-hidden">
                {/* Student header */}
                <div className="flex items-center gap-3 px-5 py-3 border-b border-stone-100 bg-stone-50 cursor-pointer hover:bg-stone-100 transition-colors"
                  onClick={() => navigate(`/students/${sid}`)}>
                  <StudentAvatar student={student} />
                  <div className="flex items-center gap-2 flex-1">
                    <span className="font-semibold text-stone-900 text-sm">{getName(sid)}</span>
                    <TierBadge tier={student?.mtss_tier} />
                    {student?.class_name && <span className="text-xs text-stone-400">{student.class_name}</span>}
                  </div>
                  <span className="text-xs text-stone-400">{intvsForStudent.length} intervention{intvsForStudent.length !== 1 ? 's' : ''}</span>
                  ...
                  {intvsForStudent.map(intv => {
                    ...
                    return (
                      <div key={intv.intervention_id}
                        onClick={() => setDetailIntv(intv)}
                        className={`flex items-center gap-4 px-5 py-3.5 cursor-pointer transition-colors ${overdue ? 'bg-red-50/50 hover:bg-red-50' : 'hover:bg-stone-50'}`}>
                        {overdue && <AlertTriangle size={13} className="text-red-400 shrink-0" />}
                        <span className="bg-stone-100 text-stone-700 text-xs px-2.5 py-1 rounded-full font-medium">{intv.intervention_type}</span>
                        <span className="text-xs text-stone-500 flex-1">{intv.assigned_staff}</span>
                        {days != null && <span className="text-xs text-stone-400 bg-stone-100 px-2 py-0.5 rounded-full">Day {days}</span>}
                        {intv.review_date && <span className={`text-xs ${overdue ? 'text-red-600 font-semibold' : 'text-stone-400'}`}>Review {intv.review_date}</span>}
                        {prog && <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${prog.cls.split('ring')[0]}`}>{prog.label}</span>}
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${intv.status === 'active' ? 'bg-teal-100 text-teal-700' : 'bg-blue-100 text-blue-700'}`}>{intv.status}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <table className="w-full text-sm group">
              <thead className="bg-stone-50 border-b border-stone-200">
                <tr>
                  ...
                  <th className="text-left py-3 px-4 text-xs font-semibold text-stone-400 uppercase tracking-wider">Staff</th>
                  ...
                  <th className="text-left py-3 px-4 text-xs font-semibold text-stone-400 uppercase tracking-wider">Progress</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-stone-400 uppercase tracking-wider">Status</th>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-5">
              <h3 className="font-bold text-stone-900 text-lg" style={{ fontFamily: 'Manrope,sans-serif' }}>New Intervention</h3>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={form.student_id} onChange={e => setForm(p => ({ ...p, student_id: e.target.value }))}
                  ...
                  className="w-full px-3 py-2.5 text-sm border border-stone-200 rounded-xl focus:outline-none bg-white">
              ...
              {professionals.length > 0 ? (
                <div>
                  <label className="text-xs font-semibold text-stone-500 uppercase tracking-wider block mb-1">Assigned Staff</label>
                ...
                <input placeholder="Assigned Staff" value={form.assigned_staff}
                  ...
                  className="w-full px-3 py-2.5 text-sm border border-stone-200 rounded-xl focus:outline-none" />
              )}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-stone-400 block mb-1">Start Date</label>
                  <input type="date" value={form.start_date} onChange={e => setForm(p => ({ ...p, start_date: e.target.value }))}
                    className="w-full px-3 py-2 text-sm border border-stone-200 rounded-xl focus:outline-none" />
                </div>
                <div>
                  <label className="text-xs text-stone-400 block mb-1">Review Date</label>
              ...
              <textarea placeholder="Reason for intervention" rows={2} value={form.rationale}
                ...
                className="w-full px-3 py-2.5 text-sm border border-stone-200 rounded-xl focus:outline-none resize-none" />
