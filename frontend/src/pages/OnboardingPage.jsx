      ...
      {steps.map((label, i) => (
        <React.Fragment key={label}>
          <div className="flex flex-col items-center gap-1">
            ...
            <span className={`text-xs font-medium whitespace-nowrap ${i === stepIndex ? 'text-stone-900' : 'text-stone-400'}`}>
              {label}
            </span>
          </div>
          {i < steps.length - 1 && (
            <div className={`flex-1 h-0.5 mx-2 mb-4 ${i < stepIndex ? 'bg-teal-300' : 'bg-stone-200'}`} />
          )}
        </React.Fragment>
      ))}
    </div>
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col items-center justify-center py-12 px-4">
      <div className="w-full max-w-xl">
        ...
        <div className="flex items-center justify-center gap-2.5 mb-8">
          ...
          <span className="font-bold text-stone-900 text-lg" style={{ fontFamily: 'Manrope,sans-serif' }}>WellTrack</span>
        </div>
        ...
        {step === 'account' && !hasAuth && (
          <div className="bg-white rounded-2xl border border-stone-200 p-8 shadow-sm">
            <h2 className="text-2xl font-bold text-stone-900 mb-1" style={{ fontFamily: 'Manrope,sans-serif' }}>
              Create your admin account
            </h2>
            <p className="text-stone-500 text-sm mb-6">This will be the primary administrator account for your WellTrack platform.</p>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={adminName} onChange={e => setAdminName(e.target.value)}
                    ...
                    className="w-full pl-10 pr-4 py-3 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-stone-900/20" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPw ? 'text' : 'password'} value={adminPassword}
                    ...
                    className="w-full pl-10 pr-10 py-3 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-stone-900/20" />
            ...
            <button onClick={handleAccountNext} data-testid="account-next-btn"
              className="mt-6 w-full bg-stone-900 text-white py-3.5 rounded-xl text-sm font-semibold hover:bg-stone-800 transition-colors flex items-center justify-center gap-2">
        ...
        {step === 'school' && (
          <div className="bg-white rounded-2xl border border-stone-200 p-8 shadow-sm">
            ...
            <p className="text-stone-500 text-sm mb-6">You can change all of these from Settings at any time.</p>

            <div className="space-y-5">
              <div>
                <label className="block text-sm font-semibold text-stone-700 mb-1.5">School Name</label>
                <input type="text" value={schoolName} onChange={e => setSchoolName(e.target.value)}
                  ...
                  className="w-full px-4 py-3 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-stone-900/20 focus:border-stone-400" />
              </div>

              <div>
                <label className="block text-sm font-semibold text-stone-700 mb-2">School Type</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-stone-700 mb-2">Current Term</label>
                <div className="grid grid-cols-4 gap-2">
                  {['Term 1', 'Term 2', 'Term 3', 'Term 4'].map(t => (
                    <button key={t} onClick={() => setCurrentTerm(t)}
                      className={`py-2.5 text-sm font-medium rounded-xl border transition-all ${currentTerm === t ? 'bg-stone-900 text-white border-stone-900' : 'bg-white text-stone-600 border-stone-200 hover:border-stone-400'}`}>
                      {t}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex gap-3 mt-6">
              {!hasAuth && (
                <button onClick={() => { setStep('account'); setError(''); }}
                  className="px-5 py-3.5 bg-stone-100 text-stone-700 rounded-xl text-sm font-medium hover:bg-stone-200 transition-colors">
                  Back
                </button>
              )}
              <button onClick={handleSchoolNext} data-testid="school-next-btn"
                className="flex-1 bg-stone-900 text-white py-3.5 rounded-xl text-sm font-semibold hover:bg-stone-800 transition-colors flex items-center justify-center gap-2">
                Continue <ArrowRight size={16} />
              </button>
            </div>
          </div>
        ...
        {step === 'data' && (
          <div className="bg-white rounded-2xl border border-stone-200 p-8 shadow-sm">
            ...
            <p className="text-stone-500 text-sm mb-6">Choose how to populate your WellTrack platform.</p>

            <div className="space-y-3 mb-6">
              <button onClick={() => setDataChoice('demo')} data-testid="data-choice-demo"
                className={`w-full text-left p-4 rounded-xl border-2 transition-all ${dataChoice === 'demo' ? 'border-stone-900 bg-stone-50' : 'border-stone-200 hover:border-stone-400'}`}>
                <div className="flex items-start gap-3">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${dataChoice === 'demo' ? 'bg-stone-900' : 'bg-stone-100'}`}>
                    <Database size={17} className={dataChoice === 'demo' ? 'text-white' : 'text-stone-500'} />
                  </div>
                  <div>
                    <p className="font-semibold text-stone-900 text-sm">Load Demo Data</p>
                    <p className="text-xs text-stone-500 mt-0.5">Sample students across 4 classes, 2 terms of screening data, interventions and analytics.</p>
                  </div>
                  {dataChoice === 'demo' && <CheckCircle size={16} className="text-teal-500 ml-auto shrink-0 mt-0.5" />}
                </div>
              </button>
              {dataChoice === 'demo' && (
                <div className="flex items-center gap-3 px-4 py-3 bg-stone-50 rounded-2xl border border-stone-200">
                  <label className="text-sm font-medium text-stone-700 shrink-0">Number of demo students:</label>
                  <input
                    ...
                    className="w-24 px-3 py-1.5 border border-stone-200 rounded-lg text-sm text-center font-medium focus:outline-none focus:ring-2 focus:ring-stone-900/20"
                  />
                  <span className="text-xs text-stone-400">(8 - 2,000)</span>
                </div>
              )}

              <button onClick={() => { setDataChoice('restore'); setTimeout(() => fileRef.current?.click(), 50); }}
                data-testid="data-choice-restore"
                className={`w-full text-left p-4 rounded-xl border-2 transition-all ${dataChoice === 'restore' ? 'border-stone-900 bg-stone-50' : 'border-stone-200 hover:border-stone-400'}`}>
                <div className="flex items-start gap-3">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${dataChoice === 'restore' ? 'bg-stone-900' : 'bg-stone-100'}`}>
                    <FileJson size={17} className={dataChoice === 'restore' ? 'text-white' : 'text-stone-500'} />
                  </div>
                  <div className="flex-1">
                    <p className="font-semibold text-stone-900 text-sm">Restore from Backup</p>
                    <p className="text-xs mt-0.5">
                      {restoreFile
                        ? <span className="text-teal-600 font-medium">{restoreFile.name} selected</span>
                        : <span className="text-stone-500">Upload a previous WellTrack JSON backup file.</span>}
                    </p>
                  </div>
                  {dataChoice === 'restore' && restoreFile && <CheckCircle size={16} className="text-teal-500 ml-auto shrink-0 mt-0.5" />}
                </div>
              </button>
              ...
              <button onClick={() => setDataChoice('blank')} data-testid="data-choice-blank"
                className={`w-full text-left p-4 rounded-xl border-2 transition-all ${dataChoice === 'blank' ? 'border-stone-900 bg-stone-50' : 'border-stone-200 hover:border-stone-400'}`}>
                <div className="flex items-start gap-3">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${dataChoice === 'blank' ? 'bg-stone-900' : 'bg-stone-100'}`}>
                    <Building2 size={17} className={dataChoice === 'blank' ? 'text-white' : 'text-stone-500'} />
                  </div>
                  <div>
                    <p className="font-semibold text-stone-900 text-sm">Start from Scratch</p>
                    <p className="text-xs text-stone-500 mt-0.5">Begin with a blank platform. Import students via CSV or add them manually.</p>
                  </div>
                  {dataChoice === 'blank' && <CheckCircle size={16} className="text-teal-500 ml-auto shrink-0 mt-0.5" />}
                </div>
              </button>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleComplete} disabled={!dataChoice || loading} data-testid="complete-onboarding-btn"
                className="flex-1 bg-stone-900 text-white py-3.5 rounded-xl text-sm font-semibold hover:bg-stone-800 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
        ...
        {/* COMPLETE STEP */}
        {step === 'complete' && (
          <div className="bg-white rounded-2xl border border-teal-200 p-10 shadow-sm text-center">
            <div className="w-16 h-16 bg-teal-100 rounded-full flex items-center justify-center mx-auto mb-5">
              <CheckCircle size={32} className="text-teal-600" />
            </div>
            <h2 className="text-2xl font-bold text-stone-900 mb-2" style={{ fontFamily: 'Manrope,sans-serif' }}>
              {schoolName} is ready!
            </h2>
            ...
            <button onClick={() => {
              ...
              className="w-full bg-stone-900 text-white py-3.5 rounded-xl text-sm font-semibold hover:bg-stone-800 transition-colors flex items-center justify-center gap-2">
