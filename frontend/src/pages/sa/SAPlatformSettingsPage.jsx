...
export default function SAPlatformSettingsPage() {
  useDocumentTitle('Platform Settings · Super Admin');
  ...
  return (
    <div className="max-w-2xl" data-testid="sa-platform-settings-page">
      ...
      {msg.text && (
        <div className={`flex items-center gap-2 rounded-xl p-3 mb-5 ${msg.type === 'error' ? 'bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/60' : 'bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/60'}`}>
          {msg.type === 'error' ? <AlertCircle size={14} className="text-rose-600 dark:text-rose-400" /> : <CheckCircle size={14} className="text-emerald-600 dark:text-emerald-400" />}
      ...
      {/* Ollama Config */}
      <div className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5 mb-5 space-y-4 transition-opacity ${!aiEnabled ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mb-2">Default: <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded text-[11px]">http://localhost:11434</code></p>
          <input
            type="text" value={ollamaUrl} onChange={e => setOllamaUrl(e.target.value)}
            data-testid="sa-ollama-url-input"
            placeholder="http://localhost:11434"
            className="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-slate-900/20"
          />
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mb-2">
            Must be pulled first: <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded text-[11px]">ollama pull {ollamaModel}</code>
          </p>
          <input
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-200 transition-colors disabled:opacity-60"
          >
            {testing ? <Loader2 size={14} className="animate-spin" /> : <Wifi size={14} />}
            {testing ? 'Testing...' : 'Test Connection'}
          </button>
          {testResult && (
            <span className={`text-xs ${testResult.connected ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
              {testResult.message}
            </span>
          )}
        </div>
