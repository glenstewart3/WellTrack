...
export default function SADashboardPage() {
  ...
  const statCards = [
    { label: 'Total Schools', value: stats?.total_schools ?? 0, icon: School, color: 'bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-900/60' },
    { label: 'Active Schools', value: stats?.active_schools ?? 0, icon: Activity, color: 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900/60' },
    { label: 'Trial Schools', value: stats?.trial_schools ?? 0, icon: Clock, color: 'bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-900/60' },
    { label: 'Total Students', value: stats?.total_students ?? 0, icon: Users, color: 'bg-violet-50 dark:bg-violet-950/30 text-violet-600 dark:text-violet-400 border-violet-200 dark:border-violet-900/60' },
  ];
  ...
  return (
    <div data-testid="sa-dashboard-page">
      ...
      {(stats?.suspended_schools > 0 || stats?.archived_schools > 0) && (
        <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/60 rounded-xl p-4 mb-6 flex items-start gap-3">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
          ...
          <Link to={`${SA_PATH_PREFIX}/schools`} className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 font-medium flex items-center gap-1">
            View all <ArrowRight size={12} />
          </Link>
        </div>
        <div className="divide-y divide-slate-100 dark:divide-slate-800" data-testid="sa-recent-schools">
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase ${styles[status] || 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`} data-testid={`status-badge-${status}`}>
      {status}
    </span>
  );
}
