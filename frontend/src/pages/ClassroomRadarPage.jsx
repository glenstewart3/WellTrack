...
export default function ClassroomRadarPage() {
  ...
  const SortHeader = ({ field, label }) => (
    <th
      ...
      className="text-left py-3 px-4 text-xs font-semibold text-stone-400 uppercase tracking-wider whitespace-nowrap cursor-pointer hover:text-stone-700 select-none"
    >
      <span className="flex items-center gap-1">
        {label}
        {sortField === field ? (
          sortDir === 'desc' ? <ChevronDown size={12} className="text-stone-700" /> : <ChevronUp size={12} className="text-stone-700" />
  ...
  const tierRowColors = { 3: 'border-l-4 border-l-red-400 bg-red-50', 2: 'border-l-4 border-l-amber-400 bg-amber-50', 1: '', 0: 'opacity-60' };
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto fade-in">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-stone-900 flex items-center gap-3" style={{fontFamily:'Manrope,sans-serif'}}>
            <Radar size={28} className="text-stone-600" /> Class Risk Radar
          </h1>
          <p className="text-stone-500 mt-1">Identify students who may need support. Click column headers to sort.</p>
        </div>
        <select
          value={selectedClass}
          onChange={e => setSelectedClass(e.target.value)}
          data-testid="class-selector"
          className="px-4 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-stone-900/20 bg-white font-medium"
        >
      ...
      {radarData.length > 0 && (
        <div className="grid grid-cols-4 gap-3 mb-6">
          {[
            { label: 'Tier 3 Students', count: tierCounts[3], color: 'border-red-200 bg-red-50', text: 'text-red-700' },
            { label: 'Tier 2 Students', count: tierCounts[2], color: 'border-amber-200 bg-amber-50', text: 'text-amber-700' },
            { label: 'Tier 1 Students', count: tierCounts[1], color: 'border-teal-200 bg-teal-50', text: 'text-teal-700' },
            { label: 'Not Screened', count: tierCounts[0], color: 'border-stone-200 bg-stone-50', text: 'text-stone-500' },
          ].map(item => (
      ...
      <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
        {loading ? (
          <div className="p-8 space-y-3">{[1,2,3,4,5].map(i => <div key={i} className="h-14 bg-stone-100 rounded animate-pulse" />)}</div>
        ) : radarData.length === 0 ? (
          <div className="p-16 text-center text-stone-400">No students found in this class.</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-stone-50 border-b border-stone-200">
                <tr>
                  ...
                  <th className="text-left py-3 px-4 text-xs font-semibold text-stone-400 uppercase tracking-wider whitespace-nowrap">Wellbeing</th>
                  <SortHeader field="attendance" label="Attendance" />
                  <SortHeader field="indicators" label="Indicators" />
                  <th className="text-left py-3 px-4 text-xs font-semibold text-stone-400 uppercase tracking-wider whitespace-nowrap">Intervention</th>
                ...
                {sortedData.map(item => {
                  ...
                  return (
                    <tr
                      ...
                      className={`border-b border-stone-100 hover:bg-stone-50 cursor-pointer transition-colors ${tierRowColors[item.mtss_tier] || ''}`}
                      data-testid={`radar-row-${item.student.student_id}`}
                    >
                      <td className="py-3.5 px-4">
                        <div>
                          <p className="font-medium text-stone-900">{item.student.first_name} {item.student.last_name}</p>
                          {item.score_trend !== null && item.score_trend !== undefined && (
                            <p className={`text-xs ${item.score_trend > 0 ? 'text-teal-600' : item.score_trend < 0 ? 'text-red-600' : 'text-stone-400'}`}>
                      ...
                      <td className="py-3.5 px-4">
                        ...
                        ) : <span className="text-xs text-stone-400">Not Screened</span>}
                      </td>
                      <td className="py-3.5 px-4">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-2">
                          <div className="w-12 bg-stone-100 rounded-full h-1.5">
                            <div className={`h-1.5 rounded-full ${item.attendance_pct < 80 ? 'bg-red-500' : item.attendance_pct < 90 ? 'bg-amber-400' : 'bg-teal-500'}`}
                              style={{ width: `${Math.min(item.attendance_pct, 100)}%` }} />
                          </div>
                          <span className={`text-xs font-medium ${item.attendance_pct < 80 ? 'text-red-600' : item.attendance_pct < 90 ? 'text-amber-600' : 'text-teal-600'}`}>
                            {item.attendance_pct}%
                          </span>
                        </div>
                      </td>
                      <td className="py-3.5 px-4">
                        <div className="flex flex-wrap gap-1 max-w-48">
                          ...
                          {item.risk_indicators.length > 3 && (
                            <span className="text-xs px-1.5 py-0.5 rounded-full bg-stone-100 text-stone-600">+{item.risk_indicators.length - 3}</span>
                          )}
                          {item.risk_indicators.length === 0 && <span className="text-xs text-stone-300">None</span>}
                        </div>
                      </td>
                      <td className="py-3.5 px-4">
                      ...
                      <td className="py-3.5 px-4 text-stone-400 text-xs">View →</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      <div className="mt-4 flex flex-wrap gap-3 text-xs text-stone-500">
        <span className="font-semibold text-stone-600">Risk Indicators:</span>
