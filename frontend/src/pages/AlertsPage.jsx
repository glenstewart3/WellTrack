...
const ALERT_TYPE_COLORS = {
  high_risk_saebrs: 'bg-red-100 text-red-700 border-red-200',
  emotional_distress: 'bg-purple-100 text-purple-700 border-purple-200',
  low_attendance_80: 'bg-red-100 text-red-700 border-red-200',
...
function AlertCard({ alert, canApprove, showResolved, onMarkRead, onResolve, onApprove, onReject, onClick }) {
  ...
  return (
    <div
      ...
      className={`bg-white border rounded-xl p-4 transition-all cursor-pointer hover:shadow-sm ${!alert.is_read ? 'border-red-200 bg-red-50/20' : 'border-stone-200'}`}
      onClick={onClick}
    >
      <div className="flex items-start gap-3">
        <div className={`mt-0.5 p-1.5 rounded-lg shrink-0 ${alert.severity === 'high' ? 'bg-red-100' : 'bg-amber-100'}`}>
          <AlertTriangle size={14} className={alert.severity === 'high' ? 'text-red-600' : 'text-amber-600'} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            <span className="font-semibold text-stone-900 text-sm">{alert.student_name}</span>
            <span className="text-xs text-stone-400">{alert.class_name}</span>
            {alert.alert_type && alert.alert_type !== 'tier_change' && (
              <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${ALERT_TYPE_COLORS[alert.alert_type] || 'bg-stone-100 text-stone-600 border-stone-200'}`}>
                {ALERT_TYPE_LABELS[alert.alert_type] || alert.alert_type}
              </span>
            )}
            {!alert.is_read && <span className="w-2 h-2 rounded-full bg-red-500 shrink-0" />}
          </div>
          <p className="text-sm text-stone-600">{alert.message}</p>
          <p className="text-xs text-stone-400 mt-1">{alert.created_at?.split('T')[0]}</p>
          {hasActions && (
            <div className="flex flex-wrap gap-1.5 mt-2" onClick={e => e.stopPropagation()}>
              {showApprove && (
                <>
                  <button onClick={() => onApprove(alert.alert_id)} data-testid={`approve-tier-${alert.alert_id}`}
                    className="text-xs px-2.5 py-1 bg-teal-50 border border-teal-200 rounded-lg text-teal-700 hover:bg-teal-100 transition-colors flex items-center gap-1">
                    <ThumbsUp size={11} /> Approve
                  </button>
                  <button onClick={() => onReject(alert.alert_id)} data-testid={`reject-tier-${alert.alert_id}`}
                    className="text-xs px-2.5 py-1 bg-red-50 border border-red-200 rounded-lg text-red-700 hover:bg-red-100 transition-colors flex items-center gap-1">
              ...
              {showMarkRead && (
                <button onClick={() => onMarkRead(alert.alert_id)}
                  className="text-xs px-2.5 py-1 bg-white border border-stone-200 rounded-lg text-stone-600 hover:bg-stone-50 transition-colors">
                  Mark Read
                </button>
              )}
              {showResolve && (
                <button onClick={() => onResolve(alert.alert_id)}
                  className="text-xs px-2.5 py-1 bg-teal-50 border border-teal-200 rounded-lg text-teal-600 hover:bg-teal-100 transition-colors">
                  Resolve
                </button>
              )}
            </div>
    ...
    <div className="p-6 lg:p-8 max-w-4xl mx-auto fade-in">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-stone-900 flex items-center gap-3" style={{fontFamily:'Manrope,sans-serif'}}>
            <Bell size={28} className="text-stone-600" /> Alerts
          </h1>
          <p className="text-stone-500 mt-1">
            {unread > 0 ? <span className="text-red-600 font-medium">{unread} unread alert{unread > 1 ? 's' : ''}</span> : 'All alerts read'}
          </p>
        </div>
        <div className="flex gap-2">
          {unread > 0 && (
            <button onClick={markAllRead} data-testid="mark-all-read-btn"
              className="px-4 py-2 text-sm font-medium bg-white border border-stone-200 rounded-xl hover:bg-stone-50 text-stone-700 transition-colors">
              Mark All Read
            </button>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          ...
          <div key={stat.label} className="bg-white border border-stone-200 rounded-xl p-4 text-center">
      ...
      {/* Tabs */}
      <div className="flex overflow-x-auto overflow-y-hidden border-b border-stone-200 mb-0 -mx-6 px-6 sm:mx-0 sm:px-0">
        {[
          ...
          <button key={key} onClick={() => setActiveTab(key)} data-testid={`alerts-tab-${key}`}
            ...
            {count > 0 && <span className="w-5 h-5 rounded-full bg-red-500 text-white text-xs flex items-center justify-center font-bold shrink-0">{count}</span>}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center justify-end mb-5 mt-3">
        <div className="flex rounded-lg border border-stone-200 overflow-hidden">
          <button onClick={() => setShowResolved(false)}
            className={`px-3 py-1.5 text-xs font-medium transition-colors ${!showResolved ? 'bg-stone-900 text-white' : 'text-stone-600 hover:bg-stone-50'}`}>
            Active
          </button>
      ...
      {activeTab === 'tier_change' && (
        <div className="space-y-3">
          {loading ? (
            [1,2,3].map(i => <div key={i} className="h-20 bg-white rounded-2xl border border-stone-200 animate-pulse" />)
            ...
            <div className="bg-white border border-stone-200 rounded-xl p-16 text-center">
              ...
              <p className="font-medium text-stone-600">No tier changes</p>
              <p className="text-sm text-stone-400 mt-1">No tier change alerts at this time</p>
            </div>
          ) : (
            tierChanges.map(alert => (
              <div key={alert.alert_id} data-testid={`alert-card-${alert.alert_id}`}
                className={`bg-white border rounded-xl p-4 transition-all ${!alert.is_read ? 'border-indigo-200 bg-indigo-50/20' : 'border-stone-200'}`}>
                <div className="flex items-start gap-3">
                  <div className={`mt-0.5 p-1.5 rounded-lg shrink-0 ${(alert.to_tier || 3) > (alert.from_tier || 1) ? 'bg-red-100' : 'bg-teal-100'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <button onClick={() => navigate(`/students/${alert.student_id}`)}
                        className="font-semibold text-stone-900 hover:text-indigo-600 transition-colors text-sm">
                        {alert.student_name}
                      </button>
                      <span className="text-xs text-stone-400">{alert.class_name}</span>
                      {alert.from_tier && alert.to_tier && (
                        <span className="flex items-center gap-1 text-xs font-medium text-stone-600">
                          ...
                          <span className={`ml-1 px-1.5 py-0.5 rounded-full text-xs font-bold ${alert.to_tier > alert.from_tier ? 'bg-red-100 text-red-700' : 'bg-teal-100 text-teal-700'}`}>
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <div className="bg-white border border-stone-200 rounded-xl p-16 text-center">
              ...
              <p className="font-medium text-stone-600">No early warnings</p>
              <p className="text-sm text-stone-400 mt-1">All clear — no active early warnings at this time</p>
            </div>
          ) : (
