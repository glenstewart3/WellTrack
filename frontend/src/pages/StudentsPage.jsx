    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-stone-100 shrink-0">
          <div>
            <h3 className="font-bold text-stone-900 text-lg" style={{fontFamily:'Manrope,sans-serif'}}>Import Students</h3>
            <p className="text-xs text-stone-400 mt-0.5">Upload a CSV file to add students in bulk</p>
          </div>
          <button onClick={onClose} className="p-1 text-stone-400 hover:text-stone-700 transition-colors"><X size={18} /></button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="flex items-center justify-between bg-stone-50 rounded-xl px-4 py-3 border border-stone-200">
            <div>
              <p className="text-sm font-semibold text-stone-700">CSV Template</p>
              <p className="text-xs text-stone-400 mt-0.5">Required: first_name, last_name, year_level, class_name, teacher</p>
            </div>
            <button onClick={downloadTemplate} data-testid="download-template-btn"
              className="flex items-center gap-2 px-3 py-2 bg-white border border-stone-200 text-stone-700 text-sm font-medium rounded-lg hover:bg-stone-50 transition-colors">
          ...
          {!result && (
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${
                dragOver ? 'border-stone-500 bg-stone-50' : 'border-stone-200 hover:border-stone-400 hover:bg-stone-50'
              }`}
            >
              <Upload size={24} className="mx-auto mb-3 text-stone-400" />
              <p className="text-sm font-medium text-stone-700">Drop your CSV file here or click to browse</p>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-semibold text-stone-700">
                  Preview — <span className="text-stone-500 font-normal">{parsed.fileName}</span>
                </p>
                <span className="text-xs bg-stone-100 text-stone-600 px-2 py-0.5 rounded-full">{parsed.rows.length} rows</span>
              </div>
              <div className="border border-stone-200 rounded-xl overflow-hidden">
                <div className="overflow-x-auto max-h-52">
                  <table className="w-full text-xs">
                    <thead className="bg-stone-50 border-b border-stone-200 sticky top-0">
                      <tr>
                        {REQUIRED_COLS.map(h => (
                          <th key={h} className="text-left py-2 px-3 font-semibold text-stone-500 whitespace-nowrap">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {parsed.rows.slice(0, 8).map((row, i) => (
                        <tr key={i} className="border-b border-stone-50">
                          {REQUIRED_COLS.map(h => (
                            <td key={h} className={`py-2 px-3 ${row[h] ? 'text-stone-700' : 'text-red-400 italic'}`}>
                              {row[h] || 'missing'}
                            </td>
                          ))}
                        </tr>
                ...
                {parsed.rows.length > 8 && (
                  <p className="text-xs text-stone-400 px-3 py-2 bg-stone-50">+{parsed.rows.length - 8} more rows not shown</p>
                )}
              </div>
            </div>
          )}
          ...
          {result && (
            <div className="space-y-3">
              <div className={`flex items-start gap-3 rounded-xl p-4 border ${result.imported > 0 ? 'bg-teal-50 border-teal-200' : 'bg-amber-50 border-amber-200'}`}>
                <CheckCircle size={18} className={result.imported > 0 ? 'text-teal-600 mt-0.5 shrink-0' : 'text-amber-600 mt-0.5 shrink-0'} />
                <div>
                  <p className={`text-sm font-semibold ${result.imported > 0 ? 'text-teal-800' : 'text-amber-800'}`}>
                ...
                <div className="border border-red-200 rounded-xl overflow-hidden">
                  ...
                  <div className="max-h-36 overflow-y-auto">
                    {result.errors.map((e, i) => (
                      <div key={i} className="flex items-center gap-2 px-3 py-2 border-b border-stone-50 last:border-0">
                        <span className="text-xs text-stone-400">Row {e.row}</span>
                        <span className="text-xs font-medium text-stone-700">{e.name || '—'}</span>
                        <span className="text-xs text-red-600 ml-auto">{e.error}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* Footer */}
        <div className="flex gap-2 px-6 py-4 border-t border-stone-100 shrink-0">
          {!result ? (
            <>
              <button onClick={doImport} disabled={!parsed || importing} data-testid="confirm-import-btn"
                className="flex-1 bg-stone-900 text-white py-3 text-sm font-semibold rounded-xl hover:bg-stone-800 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
            ...
            <button onClick={onClose} data-testid="import-done-btn"
              className="flex-1 bg-stone-900 text-white py-3 text-sm font-semibold rounded-xl hover:bg-stone-800 transition-colors">
              Done
            </button>
          )}
        </div>
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-stone-900" style={{fontFamily:'Manrope,sans-serif'}}>Students</h1>
          <div className="flex items-center gap-3 mt-1 flex-wrap">
            <p className="text-stone-500">{students.length} {filterStatus === 'archived' ? 'archived' : 'enrolled'} students</p>
            {filterStatus === 'active' && (() => {
              ...
              return chronicCount > 0 ? (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold border transition-colors ${filterChronicOnly ? 'bg-red-600 text-white border-red-600' : 'bg-red-50 text-red-700 border-red-200 hover:bg-red-100'}`}
                >
        ...
        <div className="flex gap-2">
          {filterStatus === 'active' && canDo('students.add_edit') && (
            <button onClick={() => setShowAddStudent(true)} data-testid="add-student-btn"
              className="flex items-center gap-2 px-4 py-2.5 bg-white border border-stone-200 text-stone-700 rounded-xl text-sm font-semibold hover:bg-stone-50 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-bold text-stone-900 text-lg" style={{fontFamily:'Manrope,sans-serif'}}>Add Student</h3>
              <button onClick={() => setShowAddStudent(false)}><X size={18} className="text-stone-400" /></button>
            </div>
            {addStudentError && <div className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl px-4 py-3 mb-4">{addStudentError}</div>}
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-stone-500 block mb-1">Year Level *</label>
              ...
              <div>
                <label className="text-xs text-stone-500 block mb-1">Teacher *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-stone-500 block mb-1">Gender</label>
                  <select value={addStudentForm.gender} onChange={e => setAddStudentForm(p => ({...p, gender: e.target.value}))}
                    className="w-full h-[42px] px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none">
                    <option value="">Not specified</option>
                    <option>Male</option><option>Female</option><option>Non-binary</option>
                  </select>
                </div>
                <div className="min-w-0">
                  <label className="text-xs text-stone-500 block mb-1">Date of Birth</label>
                  <input type="date" value={addStudentForm.date_of_birth} onChange={e => setAddStudentForm(p => ({...p, date_of_birth: e.target.value}))}
                    placeholder="dd/mm/yyyy"
                    className="w-full max-w-full h-[42px] px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none appearance-none" />
                </div>
              </div>
            </div>
            <div className="flex gap-2 mt-5">
              <button onClick={addSingleStudent} disabled={addStudentSaving} data-testid="confirm-add-student-btn"
                className="flex-1 bg-stone-900 text-white py-2.5 text-sm font-semibold rounded-xl hover:bg-stone-800 disabled:opacity-60 transition-colors flex items-center justify-center gap-2">
              ...
              <button onClick={() => setShowAddStudent(false)}
                className="flex-1 bg-stone-100 text-stone-700 py-2.5 text-sm font-medium rounded-xl hover:bg-stone-200 transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 mb-4">
        <span className="text-sm text-stone-500 font-medium">Status:</span>
        {[{ value: 'active', label: 'Active' }, { value: 'archived', label: 'Archived' }].map(opt => (
          <button key={opt.value} onClick={() => { setFilterStatus(opt.value); setSelectedIds(new Set()); }}
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${filterStatus === opt.value ? 'bg-stone-900 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}>
            {opt.label}
          </button>
        ))}
      </div>
      ...
      {/* Filters */}
      <div className="bg-white border border-stone-200 rounded-xl p-4 mb-6 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-36">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-stone-900/20 focus:border-stone-400"
          />
        </div>
        <select
          value={filterClass}
          ...
          className="px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-stone-900/20 bg-white hidden sm:block"
        >
                ...
                <tr>
                  ...
                  {[
                    ...
                    <th key={label}
                      ...
                      className={`text-left py-3 px-4 text-xs font-semibold text-stone-400 uppercase tracking-wider ${cls} ${key ? 'cursor-pointer select-none hover:text-stone-700 transition-colors' : ''}`}>
                      <span className="flex items-center gap-1">
                        {label}
                        {key && (sortField === key
                          ? (sortDir === 'asc' ? <ArrowUp size={12} className="text-stone-700" /> : <ArrowDown size={12} className="text-stone-700" />)
                ...
                {filtered.map(s => {
                  ...
                  return (
                    <tr
                      ...
                      className={`border-b border-stone-50 hover:bg-stone-50 cursor-pointer transition-colors ${selectedIds.has(s.student_id) ? 'bg-indigo-50/50' : ''}`}
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 bg-stone-100 rounded-full flex items-center justify-center shrink-0 overflow-hidden">
                            {s.photo_url
                              ? <img src={`${process.env.REACT_APP_BACKEND_URL}${s.photo_url}`} alt="" className="w-full h-full object-cover" />
                              : <span className="text-xs font-semibold text-stone-600">{s.first_name[0]}{s.last_name[0]}</span>
                            }
                          </div>
                          <div className="flex flex-col min-w-0">
                            <span className="font-medium text-stone-900 text-xs sm:text-sm leading-tight">
                      ...
                      <td className="py-3 px-4 text-stone-600 text-sm hidden sm:table-cell">{s.class_name}</td>
                      <td className="py-3 px-4 text-stone-600 text-sm hidden md:table-cell">{s.year_level}</td>
                      <td className="py-3 px-4">
                        {s.mtss_tier ? (
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className={`text-sm font-medium ${s.attendance_pct !== null && s.attendance_pct < 80 ? 'text-red-600' : s.attendance_pct !== null && s.attendance_pct < 90 ? 'text-amber-600' : 'text-teal-600'}`}>
                            {s.attendance_pct !== null ? `${s.attendance_pct}%` : '—'}
                          </span>
                          {s.attendance_pct !== null && s.attendance_pct < 90 && (
                            <span
                              className="inline-flex items-center gap-0.5 px-1.5 py-px rounded text-[10px] font-bold bg-red-100 text-red-700 leading-none"
      ...
      {editStudent && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-bold text-stone-900 text-lg" style={{fontFamily:'Manrope,sans-serif'}}>Edit Student</h3>
              <button onClick={() => setEditStudent(null)}><X size={18} className="text-stone-400" /></button>
            </div>
            ...
            <div className="flex flex-col items-center mb-5 gap-1.5">
              ...
              <div className="relative group">
                <button
                  ...
                  className="w-20 h-20 rounded-2xl overflow-hidden block focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-stone-400 disabled:cursor-not-allowed"
                ...
                >
                  {editStudent?.photo_url
                    ...
                    : <div className="w-full h-full bg-stone-100 flex items-center justify-center">
                        <span className="text-2xl font-bold text-stone-400">{editForm.first_name?.[0]}{editForm.last_name?.[0]}</span>
                      </div>
                  }
                  {/* hover overlay */}
                ...
                {editStudent?.photo_url && !uploadingPhoto && (
                  <button
                    ...
                    className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center transition-colors disabled:opacity-50 shadow-sm"
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs text-stone-500 block mb-1">Preferred Name</label>
      ...
      {bulkEditMode && selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-stone-900 text-white rounded-2xl px-5 py-3 flex items-center gap-4 shadow-xl">
          ...
          {filterStatus === 'archived' ? (
            canDo('students.archive') && (
              <button onClick={reactivateSelected} disabled={reactivating} data-testid="bulk-reactivate-btn"
                className="flex items-center gap-2 bg-teal-500 hover:bg-teal-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors disabled:opacity-60">
            ...
            canDo('students.archive') && (
              <button onClick={archiveSelected} disabled={archiving} data-testid="bulk-archive-btn"
                className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors disabled:opacity-60">
                {archiving ? <Loader size={13} className="animate-spin" /> : <Archive size={13} />}
                {archiving ? 'Archiving…' : 'Archive'}
              </button>
            )
          )}
          <button onClick={() => setSelectedIds(new Set())} className="text-stone-400 hover:text-white transition-colors">
            <X size={16} />
          </button>
        </div>
      )}
