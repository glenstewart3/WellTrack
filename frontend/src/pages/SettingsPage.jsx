...
import ScreeningPeriodManager from '../components/ScreeningPeriodManager';
import { useAuth } from '../context/AuthContext';
import { useSettings } from '../context/SettingsContext';
import {
  Settings, AlertTriangle, CheckCircle, Loader, School,
  Download, Upload, RefreshCw, Palette, Building2, Image, Plus, X,
  Sliders, ToggleLeft, ToggleRight, Tag, User, Users, Shield, RotateCcw, Bot, Wifi, FileUp,
...
function BrandingTab({ settings: s, onSave, saving, msg, msgType }) {
  ...
  }, [s.welcome_message, s.logo_base64, s.logo_dark_base64]);
  ...
  const handleSave = () => {
    const patch = { welcome_message: welcomeMessage, logo_base64: logoBase64, logo_dark_base64: logoDarkBase64 };
...
<button onClick={handleSave} disabled={saving} data-testid="save-branding-btn"
        className="w-full py-3.5 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-xl text-sm font-semibold hover:bg-slate-800 dark:hover:bg-white disabled:opacity-60 transition-colors flex items-center justify-center gap-2">
...
function StudentDataTab({ settings: s, onSave, saving, msg, msgType }) {
  ...
  const [schoolDayStart, setSchoolDayStart] = useState(s.school_day_start || '08:50');
  const [schoolDayEnd, setSchoolDayEnd] = useState(s.school_day_end || '15:20');
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Attendance day window */}
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        <h3 className="font-semibold text-slate-900 mb-1" style={{ fontFamily: 'Manrope,sans-serif' }}>Attendance School Day Times</h3>
        <p className="text-xs text-slate-400 mb-4">
          Upload parsing uses these values to calculate precise attended/absent fractions from late-arrival and early-leave times.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-lg">
          <div>
            <label className="text-sm font-semibold text-slate-700 mb-1 block">School Day Start</label>
            <input
              type="time"
              value={schoolDayStart}
              onChange={e => setSchoolDayStart(e.target.value)}
              data-testid="school-day-start-input"
              className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none"
            />
          </div>
          <div>
            <label className="text-sm font-semibold text-slate-700 mb-1 block">School Day End</label>
            <input
              type="time"
              value={schoolDayEnd}
              onChange={e => setSchoolDayEnd(e.target.value)}
              data-testid="school-day-end-input"
              className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none"
            />
          </div>
        </div>
...
function GeneralTab({ settings: s, onSave, saving, msg, msgType, featureFlags }) {
  ...
  useEffect(() => {
    ...
    api.get(`/settings/terms?year=${year}`).then(res => {
      const terms = res.data?.terms || [];
      const today = new Date().toISOString().split('T')[0];
      for (const t of terms) {
        if (t.start_date && t.end_date && today >= t.start_date && today <= t.end_date) {
          setDetectedTerm(t.name);
          // Auto-select if still on default or hasn't been saved yet
          if (!s.current_term) setCurrentTerm(t.name);
          return;
        }
      }
          ...
          <div className="grid grid-cols-3 gap-2">
            {[{ value: 'primary', label: 'Primary', sub: 'K–6' }, { value: 'secondary', label: 'Secondary', sub: '7–12' }, { value: 'both', label: 'K–12', sub: 'All levels' }].map(opt => (
              <button key={opt.value} onClick={() => setSchoolType(opt.value)}
                className={`p-3 rounded-xl border text-left transition-all ${schoolType === opt.value ? 'bg-slate-900 text-white border-slate-900 dark:bg-slate-800 dark:border-slate-700' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500'}`}>
                <p className="text-sm font-semibold">{opt.label}</p>
                <p className={`text-xs mt-0.5 ${schoolType === opt.value ? 'text-white/60 dark:text-slate-400' : 'text-slate-400 dark:text-slate-500'}`}>{opt.sub}</p>
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {['Term 1', 'Term 2', 'Term 3', 'Term 4'].map(t => (
              <button key={t} onClick={() => setCurrentTerm(t)}
                className={`py-2.5 text-sm font-medium rounded-xl border transition-all relative ${currentTerm === t ? 'bg-slate-900 text-white border-slate-900' : detectedTerm === t ? 'bg-blue-50 text-blue-700 border-blue-200 hover:border-blue-400' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-400'}`}>
                {t}
                {detectedTerm === t && currentTerm !== t && <span className="block text-[10px] opacity-75">Detected</span>}
              </button>
            ))}
          </div>
          {detectedTerm && detectedTerm === currentTerm && (
            <p className="text-xs text-emerald-600 mt-1.5">Auto-detected from calendar dates</p>
          )}
        </div>
        <div>
...
function ImportsTab({ msg, msgType, setMsg, setMsgType, settings, onSave }) {
  ...
  const [attendanceReplaceMode, setAttendanceReplaceMode] = useState('replace_range'); // merge | replace_range | replace_students
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await api.post('/students/import-file-preview', fd);
        if (!cancelled) setImportPreview(res.data);
      } catch (e) {
        if (!cancelled) {
          setMsgType('error');
          setMsg(e.response?.data?.detail || 'Could not preview student file');
          setTimeout(() => setMsg(''), 5000);
          setImportFile(null);
        }
      } finally { if (!cancelled) setPreviewingStudents(false); }
    })();
  ...
  const uploadAttendance = async () => {
    ...
    try {
      ...
      fd.append('replace_mode', attendanceReplaceMode);
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        <FileDropZone
          ...
          file={importFile}
          onChange={(f, v) => { setImportFile(f); setImportValid(v); setImportResult(null); }}
          testIdPrefix="import-students"
        />
        {previewingStudents && (
          <div className="mt-3 flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400" data-testid="student-preview-loading">
        ...
        {importPreview && (
          <ImportPreviewCard
            ...
            testIdPrefix="student-import-preview"
          />
        )}
        {importFile && importPreview && !previewingStudents && (
          <div className="mt-3 flex justify-end gap-2">
            <button onClick={() => { setImportFile(null); setImportPreview(null); }}
              ...
              data-testid="cancel-student-import-btn">
              Cancel
            </button>
            <button onClick={parseAndImport} disabled={importing || importValid?.ok === false} data-testid="run-import-btn"
              ...
              {importing ? 'Importing…' : `Confirm Import (${(importPreview.counts?.add || 0) + (importPreview.counts?.update || 0)} students)`}
            </button>
          </div>
        )}
        {importResult && (
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        {(() => {
          const last = settings?.attendance_last_upload_at;
          const d = daysSince(last);
          if (!last) {
            return (
              <div className="flex items-start gap-3 mb-4 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-sm" data-testid="imports-attendance-stale-banner">
                <AlertTriangle size={16} className="text-amber-500 shrink-0 mt-0.5" />
                <div className="flex-1">
                  <span className="font-semibold text-amber-800">No attendance upload found</span>
                  <span className="text-amber-700"> — upload your first attendance file below.</span>
                </div>
              </div>
            );
          }
          ...
          return (
            <div className="flex items-start gap-3 mb-4 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-sm" data-testid="imports-attendance-stale-banner">
              <AlertTriangle size={16} className="text-amber-500 shrink-0 mt-0.5" />
              <div className="flex-1">
                <span className="font-semibold text-amber-800">Attendance upload is out of date</span>
                <span className="text-amber-700"> — last upload was </span>
                <span className="font-semibold text-amber-800">{d} days</span>
                <span className="text-amber-700"> ago.</span>
                {settings?.attendance_coverage_max_date && (
                  <span className="text-amber-700"> Coverage through <span className="font-semibold text-amber-800">{settings.attendance_coverage_max_date}</span>.</span>
                )}
              </div>
            </div>
          );
        ...
        <div className="mb-4">
          <label className="text-xs font-semibold text-slate-600 block mb-1">Re-upload behaviour</label>
          <select
            value={attendanceReplaceMode}
            onChange={e => setAttendanceReplaceMode(e.target.value)}
            data-testid="attendance-replace-mode"
            className="px-3 py-2 border border-slate-200 bg-white rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-900/20"
          >
            <option value="merge">Merge (overwrite only dates in file)</option>
            <option value="replace_range">Replace date range (recommended)</option>
            <option value="replace_students">Replace all matched students (danger)</option>
          </select>
          <p className="text-xs text-slate-400 mt-1">
            Use <strong>Replace date range</strong> to prevent stale absences persisting when your export changes.
          </p>
        </div>
        <details className="mb-4 group">
          <summary className="cursor-pointer text-xs font-medium text-indigo-600 hover:text-indigo-700 select-none list-none flex items-center gap-1.5">
...
function ScreeningSessionsTab({ settings: s, onSave, saving, msg, msgType, featureFlags }) {
  ...
  return (
    <div className="space-y-6">
      {/* ── Screening Period Manager ────────────────────────────────────── */}
      <ScreeningPeriodManager />
      
      {/* ── Screening Modules ───────────────────────────────────────── */}
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        <h3 className="font-semibold text-slate-900 mb-1" style={{ fontFamily: 'Manrope,sans-serif' }}>Screening Modules</h3>
        <p className="text-xs text-slate-400 mb-5">Enable or disable screening components for your school.</p>
        
        <div className="space-y-3">
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
            <div>
              <p className="text-sm font-semibold text-slate-700">Student Self-Report</p>
              <p className="text-xs text-slate-400 mt-0.5">7-item student self-report for social, emotional, and school belonging (completed individually with student)</p>
            </div>
            <button 
              onClick={() => setModules(p => ({ ...p, saebrs_plus: !p.saebrs_plus }))}
              data-testid="module-toggle-saebrs_plus"
