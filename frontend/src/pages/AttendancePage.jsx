...
import { sortClassNames } from '../utils/classSort';
...
export default function AttendancePage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setAttMeta({
          last_upload_at: s.data.attendance_last_upload_at,
          coverage_min: s.data.attendance_coverage_min_date,
          coverage_max: s.data.attendance_coverage_max_date,
        });
  ...
  const classNames = sortClassNames([...new Set(summary.map(s => s.class_name).filter(Boolean))]);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 fade-in">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-3" style={{ fontFamily: 'Manrope,sans-serif' }}><CalendarDays size={28} className="text-slate-600" /> Attendance</h1>
      ...
      {attMeta !== null && (() => {
        const d = daysSince(attMeta?.last_upload_at);
        const stale = d != null && d > 7;
        if (!attMeta?.last_upload_at) {
          return (
            <div className="flex items-start gap-3 mb-5 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-sm" data-testid="attendance-stale-banner">
              <Info size={16} className="text-amber-500 shrink-0 mt-0.5" />
              <div className="flex-1">
                <span className="font-semibold text-amber-800">No attendance upload found</span>
                <span className="text-amber-700"> — upload an attendance file in </span>
        ...
        return (
          <div className="flex items-start gap-3 mb-5 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-sm" data-testid="attendance-stale-banner">
            ...
            <div className="flex-1">
              ...
              <span className="text-amber-700"> ago. Upload a new file in </span>
              <a href="/settings" className="font-semibold text-amber-800 underline underline-offset-2 hover:text-amber-900">
                Settings → Imports
              </a>
              <span className="text-amber-700">.</span>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6" data-testid="period-filter-row">
        ...
        {attMeta?.last_upload_at && (
          <span className="ml-auto text-xs text-slate-400">
            Last Attendance Upload — {new Date(attMeta.last_upload_at).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })}
          </span>
        )}
      </div>
                ...
                <tr key={s.student_id} onClick={() => viewStudent(s)}
                  ...
                  <td className="px-4 py-3 font-medium text-slate-900 max-w-[140px] sm:max-w-none">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full shrink-0 overflow-hidden bg-slate-100 flex items-center justify-center">
                        {s.photo_url
                          ? <img src={`${process.env.REACT_APP_BACKEND_URL}${s.photo_url}`} alt="" className="w-full h-full object-cover" />
                          : <span className="text-xs font-bold text-slate-400">{s.first_name?.[0]}{s.last_name?.[0]}</span>
                        }
                      </div>
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full shrink-0 overflow-hidden bg-slate-100 flex items-center justify-center">
                  {selectedStudent.photo_url
                    ? <img src={`${process.env.REACT_APP_BACKEND_URL}${selectedStudent.photo_url}`} alt="" className="w-full h-full object-cover" />
                    : <span className="text-sm font-bold text-slate-400">{selectedStudent.first_name?.[0]}{selectedStudent.last_name?.[0]}</span>
                  }
                </div>
                <div>
