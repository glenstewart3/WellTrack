...
function TierBadge({ tier }) {
  const c = tier === 1
    ...
    : tier === 2 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700';
  return (
...
export default function AttendancePage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto fade-in">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-stone-900" style={{ fontFamily: 'Manrope,sans-serif' }}>Attendance</h1>
        <p className="text-stone-500 mt-1 text-sm">Student attendance tracking — upload files in <strong>Settings → Imports</strong></p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6" data-testid="period-filter-row">
        ...
        {availableYears.length > 0 && (
          <div className="flex gap-1 p-1 bg-stone-100 rounded-xl" data-testid="year-selector">
            {availableYears.map(y => (
              <button key={y} onClick={() => handleYearChange(y)} data-testid={`year-btn-${y}`}
                className={`px-3 py-1.5 text-sm font-semibold rounded-lg transition-all ${
                  viewYear === y ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'
                }`}>{y}</button>
            ))}
          </div>
        )}
        ...
        <select
          ...
          className="px-3 py-2 border border-stone-200 bg-white rounded-xl text-sm font-medium text-stone-700 focus:outline-none focus:ring-2 focus:ring-stone-900/20 cursor-pointer"
        >
          {periodOptions.map(o => (
        ...
        {periodType === 'month' && (
          <input type="month" value={filterMonth} onChange={e => handleMonthChange(e.target.value)}
            ...
            className="px-3 py-1.5 border border-stone-200 bg-white rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-stone-900/20" />
        )}
        ...
        <span className="text-xs text-stone-400 font-medium">{curLabel}</span>
      </div>

      {/* Summary tiles */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Total Students', value: summary.length, sub: `with data: ${withData.length}`, icon: <Users size={18} />, color: 'text-stone-700' },
          { label: 'School Average', value: withData.length ? `${avgPct}%` : '—', sub: curLabel, icon: <CalendarDays size={18} />, color: 'text-stone-700' },
          { label: 'Tier 3 Concerns', value: concerns.length, sub: 'below 90%', icon: <TrendingDown size={18} />, color: 'text-red-600' },
          { label: 'Tier 2 At Risk', value: atRisk.length, sub: '90–95%', icon: <AlertTriangle size={18} />, color: 'text-amber-600' },
        ].map(tile => (
          <div key={tile.label} className="bg-white border border-stone-200 rounded-xl p-4">
            ...
            <p className="text-xs text-stone-400 mt-0.5">{tile.label}</p>
            <p className="text-xs text-stone-300 hidden sm:block">{tile.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-5" data-testid="cohort-filter-row">
          ...
          {[
            ...
            <button key={c.key}
              ...
              className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-all whitespace-nowrap ${
                cohortType === c.key ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'
              }`}>{c.label}</button>
          ))}
        </div>
        ...
        {cohortType === 'year' && (
          <select value={cohortValue} onChange={e => setCohortValue(e.target.value)}
            ...
            className="px-3 py-2 border border-stone-200 bg-white rounded-xl text-sm font-medium text-stone-700 focus:outline-none focus:ring-2 focus:ring-stone-900/20">
        ...
        {(cohortType !== 'school' && cohortValue) && (
          <span className="text-xs text-stone-500 font-medium">{cohortValue}</span>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search student…"
          ...
          className="px-4 py-2 border border-stone-200 rounded-xl text-sm focus:outline-none flex-1 min-w-36" />
        <select value={filterTier} onChange={e => setFilterTier(e.target.value)}
          data-testid="tier-filter"
          className="px-4 py-2 border border-stone-200 rounded-xl text-sm focus:outline-none">
      ...
      {loading ? (
        <div className="flex justify-center py-20"><Loader size={24} className="animate-spin text-stone-400" /></div>
      ) : withData.length === 0 ? (
        <div className="bg-white border border-stone-200 rounded-xl p-16 text-center">
          <CalendarDays size={40} className="mx-auto text-stone-300 mb-4" />
          <p className="text-stone-500 font-medium">No attendance data for this period</p>
          <p className="text-sm text-stone-400 mt-1">Try a different period or upload data in Settings → Imports</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm" data-testid="attendance-table">
            <thead>
              <tr className="border-b border-stone-100 text-left">
                <th className="px-4 py-3.5 text-xs font-semibold text-stone-500 uppercase tracking-wider">Student</th>
                {/* Hide class on mobile */}
                <th className="px-4 py-3.5 text-xs font-semibold text-stone-500 uppercase tracking-wider hidden sm:table-cell">Class</th>
                <th className="px-4 py-3.5 text-xs font-semibold text-stone-500 uppercase tracking-wider">Rate</th>
                {/* Hide absences on mobile */}
                <th className="px-4 py-3.5 text-xs font-semibold text-stone-500 uppercase tracking-wider hidden sm:table-cell">Absent</th>
                <th className="px-4 py-3.5 text-xs font-semibold text-stone-500 uppercase tracking-wider">Tier</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-stone-50">
              {filtered.map(s => (
                <tr key={s.student_id} onClick={() => viewStudent(s)}
                  className="hover:bg-stone-50 cursor-pointer transition-colors" data-testid={`student-row-${s.student_id}`}>
                  <td className="px-4 py-3 font-medium text-stone-900 max-w-[140px] sm:max-w-none">
                    <div className="truncate">
                      {s.first_name}{s.preferred_name && s.preferred_name !== s.first_name ? ` (${s.preferred_name})` : ''} {s.last_name}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-stone-500 hidden sm:table-cell">{s.class_name}</td>
                  <td className="px-4 py-3 min-w-[100px]">
                    {s.has_data ? <AttendancePctBar pct={s.attendance_pct} /> : <span className="text-stone-300 text-xs">No data</span>}
                  </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl max-h-[90vh] flex flex-col" data-testid="student-modal">
            <div className="flex items-center justify-between px-6 py-5 border-b border-stone-100 shrink-0">
              <div>
                ...
                <p className="text-xs text-stone-400 mt-0.5 flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 bg-stone-100 text-stone-600 rounded-full font-medium text-xs">{curLabel}</span>
                </p>
              </div>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5">
              {detailLoading ? (
                <div className="flex justify-center py-12"><Loader size={22} className="animate-spin text-stone-400" /></div>
              ) : studentDetail ? (
                <div className="space-y-5">
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-stone-50 rounded-xl p-4 text-center">
                      <p className={`text-2xl font-bold ${studentDetail.attendance_pct < 90 ? 'text-red-600' : studentDetail.attendance_pct < 95 ? 'text-amber-600' : 'text-teal-600'}`}>
                        {studentDetail.attendance_pct}%
                      </p>
                      <p className="text-xs text-stone-400 mt-0.5">Attendance</p>
                    </div>
                    <div className="bg-stone-50 rounded-xl p-4 text-center">
                      <p className="text-2xl font-bold text-stone-900">
                        {Number.isInteger(studentDetail.absent_days) ? studentDetail.absent_days : studentDetail.absent_days?.toFixed(1)}
                      </p>
                      <p className="text-xs text-stone-400 mt-0.5">Absent Days</p>
                    </div>
                    <div className="bg-stone-50 rounded-xl p-4 text-center">
                      <p className="text-2xl font-bold text-stone-900">{studentDetail.total_days}</p>
                      <p className="text-xs text-stone-400 mt-0.5">School Days</p>
                    </div>
                  </div>
                  ...
                  {studentDetail.monthly_trend?.length > 1 && (
                    <div>
                      <h4 className="text-sm font-semibold text-stone-700 mb-3">Monthly Attendance Trend</h4>
                  ...
                  {(() => {
                    ...
                    return (
                      <div>
                        <h4 className="text-sm font-semibold text-stone-700 mb-3">Absence Records ({absences.length})</h4>
                        <div className="max-h-52 overflow-y-auto space-y-1 pr-1">
                          {absences.map((r, i) => (
                            <div key={i} className="flex items-center justify-between py-1.5 px-3 bg-stone-50 rounded-lg text-xs">
                  ...
                  {Object.keys(studentDetail.absence_types || {}).length > 0 && (
                    <div>
                      <h4 className="text-sm font-semibold text-stone-700 mb-3">Absence Patterns</h4>
                            ...
                            <div key={type} className="flex items-center gap-3">
                              <div className="flex-1">
                                ...
                                <div className="w-full h-1.5 bg-stone-100 rounded-full overflow-hidden">
                                  <div className="h-full bg-stone-400 rounded-full"
                ...
                <p className="text-stone-400 text-sm text-center py-8">No attendance data for this student in this period</p>
              )}
            </div>
          </div>
        </div>
