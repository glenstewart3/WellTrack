...
import {
  ...
  CheckCircle, Stethoscope, Clock, AlertCircle, TrendingUp, ClipboardCheck, Calendar, Play,
} from 'lucide-react';
import {
...
export default function DashboardPage() {
    ...
    const load = async () => {
      try {
        ...
        const [tierRes, moveRes, alertRes, studRes, termsRes, screeningRes] = await Promise.all([
          ...
          api.get('/screening/periods/current/status').catch(() => ({ data: null })),
        ]);
        setStats(tierRes.data);
        setMovement(moveRes.data);
        setTerms(termsRes.data?.terms || []);
        setScreeningStatus(screeningRes.data);
  ...
  return (
    <div className="px-4 py-6 lg:px-8 lg:py-8 space-y-6" style={{ touchAction: 'manipulation' }}>
      ...
      {/* ── Screening Period Alerts ───────────────────────────────────── */}
      {screeningStatus && (screeningStatus.active || screeningStatus.upcoming?.length > 0) && (
        <div className="rounded-2xl border p-5" style={cardStyle}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <ClipboardCheck size={16} style={{ color: 'var(--wt-muted-fg)' }} />
              <h2 className="text-sm font-semibold" style={{ fontFamily: 'Manrope, sans-serif', color: 'var(--wt-foreground)' }}>
                Screening Periods
              </h2>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            {screeningStatus.active && (
              <button 
                onClick={() => navigate('/screening')}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors hover:opacity-90 bg-emerald-50 border border-emerald-200 text-emerald-700"
              >
                <Play size={12} /> Active: {screeningStatus.active.name} (Week {screeningStatus.active.week})
              </button>
            )}
            {screeningStatus.upcoming?.map((period, idx) => {
              const daysUntil = Math.ceil((new Date(period.start_date) - new Date(screeningStatus.today)) / (1000 * 60 * 60 * 24));
              ...
              return (
                ...
                >
                  <Calendar size={12} /> 
                  {period.name} starts in {daysUntil} day{daysUntil !== 1 ? 's' : ''}
                </button>
              );
            })}
          </div>
